...
app.post('/api/admin/hard-reset', (req, res) => {
  ...
  appState.completedLeads  = appState.completedLeads  ? {} : undefined;
  if (appState.completedLeads === undefined) delete appState.completedLeads;
  appState.followups     = appState.followups ? {} : undefined;
  if (appState.followups === undefined) delete appState.followups;
  appState.dispositions  = appState.dispositions ? [] : undefined;
  if (appState.dispositions === undefined) delete appState.dispositions;
  // Re-init blank state
  if (!appState.agents)        appState.agents        = {};
  if (!appState.allowedEids)   appState.allowedEids   = {};
  ...
  try {
    // Remove agent photos
    const photosDir = path.join(uploadsDir, 'agent_photos');
    if (require('fs').existsSync(photosDir)) {
      require('fs').readdirSync(photosDir).forEach(f => {
        try { require('fs').unlinkSync(path.join(photosDir, f)); } catch(e) {}
      });
    }
    // Remove lead docs
    const docsDir = path.join(uploadsDir, 'lead_docs');
    if (require('fs').existsSync(docsDir)) {
      require('fs').readdirSync(docsDir).forEach(f => {
        try { require('fs').unlinkSync(path.join(docsDir, f)); } catch(e) {}
      });
    }
  } catch(e) { /* non-fatal */ }
...
app.post('/api/admin/eids', (req, res) => {
  ...
  appState.allowedEids[eid] = { name: name.trim(), photo: existingPhoto || null };
  saveState(appState);
...
app.post('/api/admin/agent-photo/:eid', agentPhotoUpload.single('photo'), (req, res) => {
  try {
    ...
    if (existing) {
      const name = getEidName(existing);
      appState.allowedEids[eid] = { name, photo: req.file.path };
    } else {
      appState.allowedEids[eid] = { name: '', photo: req.file.path };
    }
    saveState(appState);
