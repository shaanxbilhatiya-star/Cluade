...
app.post('/api/admin/hard-reset', (req, res) => {
  ...
  // Delete all files in uploads/lead_docs and uploads/agent_photos
  try {
    const leadFiles = fs.readdirSync(LEAD_DOCS_DIR);
    leadFiles.forEach(f => { if (f !== '.gitkeep') { try { fs.unlinkSync(path.join(LEAD_DOCS_DIR, f)); } catch {} } });
  } catch {}
  try {
    const photoFiles = fs.readdirSync(AGENT_PHOTOS_DIR);
    photoFiles.forEach(f => { if (f !== '.gitkeep') { try { fs.unlinkSync(path.join(AGENT_PHOTOS_DIR, f)); } catch {} } });
  } catch {}
  saveState(appState);
  io.emit('force-stop');
  broadcastAdminStats();
