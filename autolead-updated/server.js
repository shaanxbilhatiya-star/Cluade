...
function createFreshState() {
  return {
    ...
    uploadedFiles: [],
    dialedLog: [],
    dispositions: [], // Track all disposition actions
...
// Migrate old states without dispositions array
if (!appState.dispositions) {
  appState.dispositions = [];
}
...
// ─── Number helpers ───────────────────────────────────────────────────────────
function canDialNumber(num) {
  if (!num) return false;
  
  // 1. Call Not Connected (Dead number) - never dial again
  if (num.disposition === 'DEAD') return false;
  
  // 3. Not Interested - don't dial for 30 days
  if (num.disposition === 'NOT_INTERESTED' && num.dispositionDate) {
    const daysSince = Math.floor((Date.now() - new Date(num.dispositionDate).getTime()) / (1000 * 60 * 60 * 24));
    if (daysSince < 30) return false;
  }
  
  // 4. Followup - only dial if followup date/time has passed and not yet dialed for followup
  if (num.disposition === 'FOLLOWUP' && num.followupDateTime) {
    const followupTime = new Date(num.followupDateTime).getTime();
    const now = Date.now();
    // If followup time hasn't arrived yet, can't dial
    if (now < followupTime) return false;
    // If already dialed for this followup, don't dial again
    if (num.followupDialed) return false;
  }
  
  // 6. Interested - already pinned, don't keep dialing
  if (num.disposition === 'INTERESTED') return false;
...
function getNextNumber(agentId) {
  ...
  // Priority 1: Followups that are due
  const dueFollowup = appState.numbers.find(n => 
    n.disposition === 'FOLLOWUP' && 
    n.followupDateTime &&
    !n.followupDialed &&
    new Date(n.followupDateTime).getTime() <= Date.now() &&
    !n.assignedTo &&
    canDialNumber(n)
  );
  if (dueFollowup) {
  ...
  // Priority 2: Numbers without disposition or Call Not Received / Switch OFF from yesterday
  const undialed = appState.numbers.find(n => 
    !n.assignedTo && 
    canDialNumber(n) &&
    (
      !n.disposition || 
      n.disposition === 'NOT_RECEIVED' || 
      n.disposition === 'SWITCH_OFF'
    )
  );
...
function markDialed(agentId, numberId, disposition, followupDateTime) {
  appState = checkDailyReset(appState);
  ...
  // Set disposition
  num.disposition = disposition;
  num.dispositionDate = timestamp;
  num.dispositionBy = agentId;
  
  // Handle specific dispositions
  if (disposition === 'FOLLOWUP' && followupDateTime) {
    num.followupDateTime = followupDateTime;
    num.followupDialed = false; // Reset this flag for new followup
  } else if (disposition === 'FOLLOWUP' && num.followupDateTime) {
    // This is a followup call being made
    num.followupDialed = true;
  }
  
  if (disposition === 'INTERESTED') {
    num.interested = true;
    num.interestedAt = timestamp;
  }
  
  // Mark as dialed (contacted)
  num.dialedBy = agentId;
  num.dialedAt = timestamp;
  num.assignedTo = null;
  ...
  appState.dialedLog.push({
    ...
    disposition,
    timestamp,
    followupDateTime: followupDateTime || null
  });
  
  // Log to dispositions array for admin tracking
  appState.dispositions.push({
    id: uuidv4(),
    numberId: num.id,
    phone: num.phone,
    name: num.name || '',
...
function getAdminStats() {
  ...
  const remaining = appState.numbers.filter(n => canDialNumber(n) && !n.assignedTo).length;

  // Disposition breakdown
  const dispositionStats = {
    DEAD: appState.numbers.filter(n => n.disposition === 'DEAD').length,
    NOT_RECEIVED: appState.numbers.filter(n => n.disposition === 'NOT_RECEIVED').length,
    NOT_INTERESTED: appState.numbers.filter(n => n.disposition === 'NOT_INTERESTED').length,
    FOLLOWUP: appState.numbers.filter(n => n.disposition === 'FOLLOWUP' && !n.followupDialed).length,
    SWITCH_OFF: appState.numbers.filter(n => n.disposition === 'SWITCH_OFF').length,
    INTERESTED: appState.numbers.filter(n => n.disposition === 'INTERESTED').length
  };
  
  // Interested leads (pinned)
  const interestedLeads = appState.numbers
    .filter(n => n.disposition === 'INTERESTED')
    .map(n => ({
      id: n.id,
      phone: n.phone,
      name: n.name || '',
      agentName: appState.agents[n.dispositionBy]?.name || n.dispositionBy || 'Unknown',
      timestamp: n.interestedAt || n.dispositionDate
    }))
    .sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
  
  // Scheduled followups
  const scheduledFollowups = appState.numbers
    .filter(n => n.disposition === 'FOLLOWUP' && n.followupDateTime && !n.followupDialed)
    .map(n => ({
      id: n.id,
      phone: n.phone,
      name: n.name || '',
      ...
      isDue: new Date(n.followupDateTime).getTime() <= Date.now()
    }))
  ...
  return { 
    total, dialed, assigned, remaining, 
    dispositionStats, 
    interestedLeads,
    scheduledFollowups,
    agentStats, 
...
io.on('connection', (socket) => {
  ...
  socket.on('agent-disposition', ({ agentId, numberId, disposition, followupDateTime }) => {
    appState = checkDailyReset(appState);
    const agent = appState.agents[agentId];
    if (!agent) return socket.emit('error', 'Agent not found');

    if (numberId) markDialed(agentId, numberId, disposition, followupDateTime);
