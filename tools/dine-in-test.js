...
async function run() {
  ...
  try {
    ...
    const home = await api('GET', '/api/dine-in/rangoli', { token });
    ...
    const walkQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    ...
    let expectedBills = 0;
    let expectedSaved = 0;
    const walkBill = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 2000, payment: { method: 'upi' }, tableNumber: '7' } });
    ...
    const slots = await api('GET', '/api/dine-in/rangoli/slots', { token });
    ...
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { arriveEarlyMinutes: 1440 } });

    const made = await api('POST', '/api/dine-in/rangoli/reservations', {
      token,
    ...
    const lockedQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    ...
    const lockedPay = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 2000, payment: { method: 'upi' } } });
    ...
    const fallbackQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000, mode: 'walkin' } });
    ...
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 0, arriveEarlyMinutes: 1440 } });
    const unlocked = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    ...
    const reservedPay = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 2000, payment: { method: 'card' } } });
    ...
    const reused = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    ...
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 30, arriveEarlyMinutes: 30 } });
    ...
    if (process.env.DINE_SLOW_TEST === '1') {
      ...
      await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 1, arriveEarlyMinutes: 1440 } });
      const slow = await api('POST', '/api/dine-in/rangoli/reservations', {
        token,
      ...
      const beforePay = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 1000, payment: { method: 'upi' } } });
      ...
      await sleep(65000);
      const afterWait = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000 } });
      ...
      const afterPay = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 1000, payment: { method: 'upi' } } });
    ...
    var farSlots = await api('GET', '/api/dine-in/rangoli/slots?date=' + farDate, { token });
    var farSlot = farSlots.body.slots[0];
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 0, arriveEarlyMinutes: 30 } });
    var farRes = await api('POST', '/api/dine-in/rangoli/reservations', {
      token,
    ...
    var farQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000, reservationId: farRes.body.reservation.id } });
    check('quoting against it cannot be paid at the reserved rate', farQuote.body.canPayNow === false);
    var farPay = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 1000, reservationId: farRes.body.reservation.id, payment: { method: 'upi' } } });
    ...
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 30 } });
    ...
    var mineAgain = await api('GET', '/api/dine-in/rangoli/reservations', { token });
    var someReservation = (mineAgain.body.reservations || [])[0];
    if (someReservation) {
      var poke = await api('POST', '/api/dine-in/rangoli/quote', { token: otherToken, body: { billAmount: 1000, reservationId: someReservation.id } });
    ...
    var anonQuote = await api('POST', '/api/dine-in/rangoli/quote', { body: { billAmount: 1000, reservationId: reservation.id } });
    ...
    var huge = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1e21 } });
    check('an astronomical bill is rejected', huge.status === 400, `status ${huge.status}`);
    var overMax = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 500000 } });
    check('a bill over the configured maximum is rejected', overMax.status === 400 && /counter/i.test(overMax.body?.error || ''));
    var arrayAmount = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: [500] } });
    check('a non-scalar amount is rejected', arrayAmount.status === 400, `status ${arrayAmount.status}`);
    var boolAmount = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: true } });
    ...
    var spoof = await api('POST', '/api/dine-in/rangoli/quote', {
      token,
    ...
    const adminView = await api('GET', '/api/admin/dine-in/rangoli', { token: adminToken });
    ...
    const changed = await api('PUT', '/api/admin/dine-in/rangoli/settings', {
      token: adminToken,
    ...
    const afterChange = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    ...
    const rewritten = await api('PUT', '/api/admin/dine-in/rangoli/settings', {
      token: adminToken,
    ...
    const customQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000 } });
    ...
    const reset = await api('POST', '/api/admin/dine-in/rangoli/notices/reset', { token: adminToken });
    ...
    const capped = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { reservedDiscountPercent: 30, walkinDiscountPercent: 10, lockMinutes: 30, maxDiscountAmount: 300 } });
    check('a rupee cap can be set on the instant discount', capped.body.settings.maxDiscountAmount === 300);
    const cappedQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 2000 } });
    check('the cap is honoured: 10% of 2000 is 200, under the 300 cap', cappedQuote.body.amounts.discount === 200);
    const bigQuote = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 6000 } });
    ...
    const stacked = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 6000, offerCode: 'DINETEST' } });
    ...
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { maxDiscountAmount: 0 } });
    const uncapped = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000, offerCode: 'DINETEST' } });
    ...
    const clamped = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { reservedDiscountPercent: 250 } });
    check('an out-of-range discount is clamped to 100', clamped.body.settings.reservedDiscountPercent === 100);
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { reservedDiscountPercent: 30 } });

    const badBasis = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockBasis: 'whenever' } });
    ...
    const inverted2 = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { walkinDiscountPercent: 80 } });
    ...
    const deadEnd = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { lockMinutes: 1200, graceHours: 0, arriveEarlyMinutes: 0 } });
    ...
    const badTime = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { openTime: '25:99' } });
    check('an invalid opening time is rejected', badTime.status === 400);
    const inverted = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { openTime: '22:00', closeTime: '10:00' } });
    check('closing before opening is rejected', inverted.status === 400);

    const asCustomer = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token, body: { reservedDiscountPercent: 99 } });
    check('a customer cannot change the discounts', asCustomer.status === 403, `status ${asCustomer.status}`);
    const anon = await api('POST', '/api/dine-in/rangoli/bills', { body: { billAmount: 500 } });
    check('paying a bill requires sign-in', anon.status === 401, `status ${anon.status}`);
    const zero = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 0 } });
    check('a zero bill is rejected', zero.status === 400);
    const negative = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: -500 } });
    check('a negative bill is rejected', negative.status === 400);

    const minimum = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { minBillAmount: 500 } });
    check('a minimum bill can be configured', minimum.body.settings.minBillAmount === 500);
    const tooSmall = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 200 } });
    check('a bill under the minimum is rejected', tooSmall.status === 400 && /cannot be paid/i.test(tooSmall.body?.error || ''));
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { minBillAmount: 0 } });

    const otherReservation = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000, reservationId: 'dres_nope' } });
    check('an unknown reservation id is a 404', otherReservation.status === 404);

    const bigParty = await api('POST', '/api/dine-in/rangoli/reservations', {
      token,
    ...
    const pastDate = await api('POST', '/api/dine-in/rangoli/reservations', {
      token,
      body: { date: '2020-01-01', time: slot.time, partySize: 2, guestName: 'Andrew' },
    });
    check('a past date is rejected', pastDate.status === 400);

    const disabled = await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { active: false } });
    check('the whole tab can be switched off', disabled.body.settings.active === false);
    const whileOff = await api('POST', '/api/dine-in/rangoli/quote', { token, body: { billAmount: 1000 } });
    check('quotes are refused while the tab is off (503)', whileOff.status === 503, `status ${whileOff.status}`);
    await api('PUT', '/api/admin/dine-in/rangoli/settings', { token: adminToken, body: { active: true } });

    // ── Pay at the counter ───────────────────────────────────────────────────
    /* 'cash' is money not yet collected, so the bill must not claim to be paid. */
    section('Pay at the counter is not settled money');
    const cashBill = await api('POST', '/api/dine-in/rangoli/bills', { token, body: { billAmount: 1000, payment: { method: 'cash' } } });
    ...
    const billedRes = db_findBilledReservation(await api('GET', '/api/admin/dine-in/rangoli', { token: adminToken }));
    if (billedRes) {
    ...
    // ── Receipts ─────────────────────────────────────────────────────────────
    section('Receipts');
    const bills = await api('GET', '/api/dine-in/rangoli/bills', { token });
